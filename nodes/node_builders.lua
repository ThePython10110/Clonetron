...
local builder_on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
	...
	minetest.show_formspec(clicker:get_player_name(),
		"clonetron:builder"..minetest.pos_to_string(pos),
		builder_formspec(pos, meta))
end

minetest.register_on_player_receive_fields(function(sender, formname, fields)

	if formname:sub(1, 15) ~= "clonetron:builder" then
		return
	end
	...
	if extrusion and extrusion > 0 and extrusion <= clonetron.config.maximum_extrusion then
	...
	elseif fields.read then
		...
		local buildpos = clonetron.find_new_pos(pos, facing)
		local target_node = minetest.get_node(buildpos)
		if target_node.name ~= "air" and minetest.get_item_group(target_node.name, "clonetron") == 0 then
			local meta = minetest.get_meta(pos)
			local inv = meta:get_inventory()
			local target_name = clonetron.builder_read_item_substitutions[target_node.name] or target_node.name
	...
	if fields.help and minetest.get_modpath("doc") then --check for mod in case someone disabled it after this digger was built
		minetest.after(0.5, doc.show_entry, sender:get_player_name(), "nodes", "clonetron:builder", true)
	end

	clonetron.update_builder_item(pos)
end)
...
minetest.register_node("clonetron:builder", {
	...
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		...
		if not stack_def and not clonetron.whitelisted_on_place(stack_name) then
	...
	test_build = function(pos, test_pos, inventory_positions, protected_nodes, nodes_dug, controlling_coordinate, controller_pos)
		local meta = minetest.get_meta(pos)
		local facing = minetest.get_node(pos).param2
		local buildpos = clonetron.find_new_pos(test_pos, facing)
		...
		while extrusion_count < extrusion_target do
			...
			local source_location = clonetron.take_from_inventory(item_stack:get_name(), inventory_positions)
			...
			buildpos = clonetron.find_new_pos(buildpos, facing)
		end
	...
	execute_build = function(pos, player, inventory_positions, protected_nodes, nodes_dug, controlling_coordinate, controller_pos)
		...
		while extrusion_count < extrusion_target do
			if not clonetron.can_build_to(buildpos, protected_nodes, nodes_dug) then
				return built_count
			end
			...
			local sourcepos = clonetron.take_from_inventory(item_stack:get_name(), inventory_positions)
			...
			if success == true then
				...
				clonetron.place_in_specific_inventory(item_stack, sourcepos, inventory_positions, controller_pos)
				return built_count
			end
