...
local function check_clonetron_size(layout)
	if #layout.all > clonetron.config.size_limit then
		return S("Size limit of @1 reached with @2 nodes!", clonetron.config.size_limit, #layout.all)
	end
end
...
clonetron.execute_dig_cycle = function(pos, clicker)
	...
	local size_check_error = check_clonetron_size(layout)
	if size_check_error then
		return pos, size_check_error, 8
	end
	
	local controlling_coordinate = clonetron.get_controlling_coordinate(pos, facing)
	...
	for _, location in pairs(layout.all) do
		...
		if not clonetron.can_move_to(newpos, layout.protected, layout.nodes_dug) then
			can_move = false
		end
	end
		...
		for k, location in pairs(layout.builders) do
			...
			if targetdef.test_build ~= nil then
				...
				for k, return_item in pairs(test_build_return_items) do
					...
					test_build_fuel_cost = test_build_fuel_cost + clonetron.config.build_cost
				end
	...
	if minetest.get_modpath("technic") then
		if layout.power_connectors ~= nil then
			...
			power_from_cables = power_from_cables / clonetron.config.power_ratio
			test_fuel_burned = power_from_cables
		end
		
		if test_fuel_needed - test_fuel_burned > 0 then
			-- check for the available electrical power
			test_fuel_burned = test_fuel_burned + clonetron.tap_batteries(layout.battery_holders, test_fuel_needed, true)
		end
	end
	if (fuel_burning<0) then
		test_fuel_needed = test_fuel_needed - fuel_burning
	...
	if (test_fuel_needed < test_fuel_burned) then
		...
		test_fuel_burned = test_fuel_burned + clonetron.burn(layout.fuelstores, test_fuel_needed - test_fuel_burned, true)
		exhaust = 1 -- burning fuel produces smoke
	end
	
	--Put everything back where it came from
	for k, item_return in pairs(test_items) do
		clonetron.place_in_specific_inventory(item_return.item, item_return.location, layout.inventories, layout.controller)
	end
	...
	if clonetron.config.damage_hp > 0 and layout.diggers ~= nil then
		for k, location in pairs(layout.diggers) do
			local target = minetest.get_node(location.pos)
			local targetdef = minetest.registered_nodes[target.name]
	...
	for _, itemname in pairs(items_dropped) do
		clonetron.place_in_inventory(itemname, layout.inventories, oldpos)
	end
	clonetron.award_item_dug(items_dropped, clicker) -- Achievements mod hook
	...
	if layout.builders ~= nil then
		for k, location in pairs(layout.builders) do
			...
			if targetdef.execute_build ~= nil then
				...
				elseif clonetron.config.uses_resources then
					building_fuel_cost = building_fuel_cost + (clonetron.config.build_cost * build_return)
				end
			else
	...
	if clonetron.config.particle_effects and exhaust == 1 then
		table.insert(particle_systems, burn_smoke(pos, total_fuel_cost))
	end
	if fuel_burning < 0 then
		-- we tap into the batteries either way
		fuel_burning = fuel_burning + clonetron.tap_batteries(layout.battery_holders, -fuel_burning, false)
		if exhaust == 1 then
			-- but we burn coal only if we must (exhaust = flag)
			fuel_burning = fuel_burning + clonetron.burn(layout.fuelstores, -fuel_burning, false)
		end
	end
...
-- 3 - obstructed by undiggable node
-- 4 - insufficient fuel
clonetron.execute_downward_dig_cycle = function(pos, clicker)
	local meta = minetest.get_meta(pos)
	local facing = minetest.get_node(pos).param2
	local dir = clonetron.facedir_to_down_dir(facing)
	local fuel_burning = meta:get_float("fuel_burning") -- get amount of burned fuel left over from last cycle
	local status_text = S("Heat remaining in controller furnace: @1", math.floor(math.max(0, fuel_burning)))
	local exhaust = meta:get_int("on_coal")
	...
	--move the array
	layout:move_layout_image(clonetron.facedir_to_down_dir(facing))
	if not layout:write_layout_image(clicker) then
		return pos, "unrecoverable write_layout_image error", 1
	end
	local oldpos = {x=pos.x, y=pos.y, z=pos.z}
